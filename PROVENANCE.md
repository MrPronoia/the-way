...
| `.github/workflows/deploy-site.yml`                      | *(intentionally not copied)*                             | The private mr-pronoia build pipeline. Excluded — it discloses the upstream private repo's internals and serves no purpose here. The architecture is documented in prose in `jesus-site-reference/HOW-ITS-BUILT.md` instead. |
